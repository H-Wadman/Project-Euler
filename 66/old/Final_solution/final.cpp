...
class Fraction
{
    std::uint64_t m_num;  //Numerator of our fraction
    std::uint64_t m_den;  //Denominator of our fraction

public:
    Fraction(std::uint64_t num = 0, std::uint64_t den = 1) : m_num {num}, m_den {den} {};

    std::uint64_t getNum() { return m_num; }
    std::uint64_t getDen() { return m_den; }
...
Fraction operator+(Fraction f1, Fraction f2)
{
    std::uint64_t den {f1.getDen() * f2.getDen()};
    std::uint64_t num {f1.getNum() * f2.getDen() + f2.getNum() * f1.getDen()};
    /* 
...
{   
    ...
    while(true)
    {
        ...
        if(res.getNum() * res.getNum() - d * res.getDen() * res.getDen() == 1) //x^2 - D * y^2 = 1
            return res;
